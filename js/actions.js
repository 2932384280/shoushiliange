...
export function resolveExplore(place, action) {
    ...
    if (place.type === 'guyhome') {
        ...
        if (hg && !hg.locked && !hg.banished) {
            ...
            if (action === '🏠拜访') {
                // 拜访逻辑由 handleGuyHomeVisit 处理，但如果在 resolveExplore 中调用，则特殊处理
                // 此处为兼容，实际由 openPlaceActions 调用 handleGuyHomeVisit
